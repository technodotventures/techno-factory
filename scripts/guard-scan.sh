...
# 7. Design check — UI-impacting changes need a brand-fidelity verdict (ADR 0007).
#    Detection only: the JUDGMENT stays with the design reviewer (repo-resident
#    product-design skill). The builder may not self-approve.
UI_HIT=$(printf '%s\n' "$CHANGED" | grep -E '(\.tsx|\.jsx|\.css|\.scss|\.sass|\.less|\.html|\.vue|\.svelte|\.svg)$|(^|/)(design-system|design-tokens)(/|$)' || true)
[ -n "$UI_HIT" ] && flag "UI-impacting change — design check required (attach brand-fidelity + agentic-UX pattern verdicts — docs/design-principles.md — via the repo-resident product-design skill before Review; builder may not self-approve)"
