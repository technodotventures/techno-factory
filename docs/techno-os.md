...
**Peers, not modules:** Coffee (collaborative surface — source of record for human work state), Smartware/Pod (memory substrate), expresso (workflow semantics), GitHub/GitLab (delivery), agent runtimes (external, Coffee-native, BYO).
...
| **External agents** (personal agents, BYO) | invited periphery: same 129-tool surface, own identity via OAuth | Fully capable — OAuth client `hermes` holds the full scope set (`coffeeConnect.*.all`) |
| **External MCP/API** | the wiring under everything: notifications, evidence, automation | 129 tools live |

**Plane split (Sep 13 direction):** Coffee is the **control plane** — state, agents, triggers, gates, evidence, notifications — plus, opt-in per project, a **repo lane** (GitHub App broker: isolated `coffee/` branches, file edits, PRs, CI reads). Code execution stays an **external plane**: sandboxed workers run builds/checks and report back through `/agent-execution` (status projections, questions, exact-action approvals). *Identity/reasoning in Coffee, execution external* — the product's own architecture; the factory adopts it. Live probe Sep 13: **status-event workflows fire end-to-end** — an API-created workflow ran a reviewer step seconds after its card entered IN REVIEW (immutable root-task snapshot in the step prompt = trust-stamped scope, native). The Techno OS repo lane awaits a UI connect; execution dispatch requires a registered executor manifest.
...
**Known gaps (tracked):** checklist verdicts can't be enumerated via API (no read tool — ticks work when the id is known; asked) · `task.activity` webhook delivery unverified · approval-band UI is a Coffee-side proposal (interim: status + evidence comment). **Resolved Sep 13:** named agent identity (`acting_agent_id` on 135 tools — `via_agent` attribution live); IN REVIEW status added; surface grew to 136 tools (agent CRUD, artifacts, file links).
...
**Three layers, one authority (Spotify Portal pattern):** hooks — machine checks, the only layer that can *refuse*; scripts — plumbing the model never handles (transport, token refresh, scans); skills/prompts — advisory by design; they redirect, they never enforce. Every hard guard lives in the hook tier; a rule that exists only in a prompt is a suggestion.

**Evidence is never model-summarized.** Context reads may be routed through cheap workers (bulk-read → bullets), but evidence artifacts — test output, diffs, recordings, verdicts — stay byte-exact and primary. A summary is a claim; the mission question needs the artifact.
...
**Live today:** full Coffee connection (129 tools, proven round-trips); native agents task-capable; Discovery Interview pattern ready; `coffee-notify` posting run updates to the "Techno OS" channel; docs canonical (this file + `pdlc.md` + `coffee-integration.md` + `templates/`).

**Next increments (in order):**
1. ~~Create the **Factory Planner** native agent~~ ✅ live — spec'd its first two cards natively (Sep 13); the Spec station runs.
...
4. **Production hosting** — move the loop to a venture-owned host (worker profile + watcher daemon + venture credentials), demoting personal environments to optional operators. Checklist + adapter contract: `docs/portability.md`.
