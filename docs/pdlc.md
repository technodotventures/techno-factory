...
**Run close — verdicts are explicit.** Every criterion carries a verdict before a card is COMPLETE: a **tick** (met, evidence-linked in the run log) or an explicit note (not met / waived, with reason). Ticks carry agent attribution (`via_agent`). A COMPLETE card with silently unticked criteria is a broken receipt, not a closed run. *Mechanics (Sep 13): ticking is API-driveable (`PatchTaskChecklistByTaskChecklistId {is_checked}` — verified) when the factory captured the item id at creation (`task_checklist_id` is returned by create); pre-existing items can't be enumerated until the checklist-read tool is exposed — tracked in `coffee-integration.md`.*
