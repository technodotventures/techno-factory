...
- **Example native pattern (live today):** the **Discovery Interview** — a native agent joins a Coffee meeting room, interviews a client/founder live (one question at a time), records + transcribes, and produces a Client Discovery Brief that feeds Spec. Meetings + agents + transcription all exist natively; this is the first PDLC stage that runs fully on Coffee primitives today. Reference prompt: `docs/templates/discovery-interviewer-agent.md`.
...
| **MCP-capable surface (platform)** | **Partially live (Sep 11, dogfood finding #1):** a custom native agent CAN add checklist items to a real task ✓, but task-**create** and **status-change** fail with *workspace validation errors*; Coffee AI's own task creation works ✓ (platform capability exists; the custom-agent tooling is the gap — Coffee team updating). Likely cause: `POST /task` requires `task_project_status[{project_hash, task_status_hash}]` — both hashes must resolve; agent tooling appears to send unresolved values. | Fix custom-agent task tools (hash resolution); enumerate the exact in-app tool catalogue; file upload; map factory stations onto native agents | Grant task/workspace scopes to external clients too |
...
## 6c. Dogfood finding #1 (Sep 11) — custom-agent task ops + repro for the Coffee team

**What works:** checklist-add on an existing task via custom agent ✓ (verified live); task creation via **Coffee AI** ✓ (built-in path — platform capability exists).
**What fails:** task **create** via custom agent → *"workspace validation error"*; task **status-change** ("move to complete") → *"workspace rejected the update with [validation] error."*
**Likely cause (from the public spec):** `POST /task` requires `task_project_status: [{project_hash, task_status_hash}]` — both hashes must resolve (and the status must belong to that project's workflow). The agent's tooling appears to send unresolved names/stale values. Suggested fix direction: task tools resolve hashes first via `GET /workspace/project` + `GET /task/status` per project.
**Repro prompts:** (1) *"Create a new task in Optimus OS titled 'Design token usage widget' with description '…'"* → validation error; (2) *"move [task link] to complete"* → validation error.
