...
- **#4 — RESOLVED (Sep 16):** checklist read + delete shipped over MCP — `GetTaskChecklist` returns `{task_checklist_id, title, is_checked, order}` (verified on run-008's card: 8/8 items read) and `DeleteTaskChecklistByTaskChecklistId` is live; enumeration is no longer capture-at-create-only.

**Filing convention (Sep 13):** technical requests are filed as tasks in the **Coffee Bugs 🐞** project, assigned to the Coffee dev. Open asks (updated **Sep 16**): content search (workspace modules + messenger full-text + reliable `GetTask` filters — filed Sep 16) · `task.activity` webhook re-verify `redacted-id-04` · status automations API `redacted-id-05` (deprioritized Sep 13) · status audit `redacted-id-06` · task-ID menu `redacted-id-07` · steering lane `redacted-id-08` · note/document paging `redacted-id-09` · owner-neutral task create `redacted-id-10` · OAuth-readable files `redacted-id-11` · agent presence on cards `redacted-id-12` · MCP response envelopes `redacted-id-13` · approval CTAs (decision band + queue) `redacted-id-14` · project sharing/participants `redacted-id-15` · root-task access for workflow steps `redacted-id-16` · status delete/archive `redacted-id-17` · project templates / workflow copy `redacted-id-18` · agent-assignment completion state (API-created assignments stay `staged` after the run finishes; UI ones go `done`) `redacted-id-19`.
...
**Pipeline:** statuses **TO DO · READY · IN PROGRESS · IN REVIEW · COMPLETE** (a card's status IS the stage; READY = specced + dispatchable). Stations: **Spec** (Factory Planner) and **Review** (Factory Tester) — both run on the native assignment lane (below). One shared **Factory Planner** serves every line; per-project **`agent_brief`** carries each line's repo / conventions / definition-of-done into delegated runs.

**Station lanes — v2, native standard (Sep 17; canary + live-card verified):** stations run on Coffee's **native assignment lane**. `scripts/spec-advance.py` (cron, every 2 min) assigns the **Factory Planner** to fresh TO DO cards — the plan comment + acceptance criteria land **ON the card** — and advances TO DO → **READY** (+ channel notify) once criteria exist; cards entering **IN REVIEW** get the **Factory Tester** assigned the same way, and its evidence-linked TEST REPORT lands **on the card** (the tester reads the whole card — comments and checklist IDs included; the §6j evidence-reachability gap does not apply on this lane). History: the Sep-15 workflow-step lane (sandboxed step tasks, deliverables off-card) is **DISABLED on all six lines** — kept in `factory-pipeline.json` v4 for rollback (applier owns workflow enablement). Known cosmetic quirk: API-created assignments stay `staged` after completion (UI ones go `done`) — advance logic keys off the deliverable (criteria present), not the status; platform ask filed.

## 6j. Station evidence delivery — the legacy step lane (Sep 16; superseded Sep 17 by the native lane — kept for the disabled step lane)

**(Superseded Sep 17: the Review station now runs on the native assignment lane — the tester reads the card directly, so evidence no longer needs to ride the Description. Keep this section only for the disabled legacy step lane.)**
...
6. **Factory lines (Sep 15 → Sep 17):** six lines provisioned; stations standardized on the **native assignment lane** (Sep 17; §6i) — root-card projection now happens natively; the step-run root-access ask (`redacted-id-16`) remains open for the legacy lane only.
7. **Station evidence delivery (Sep 16):** superseded Sep 17 — native-lane reviewers read the card directly (§6i); §6j kept for the disabled step lane.
