...
6. The Verifier is blind to Builder reasoning and re-runs the checks itself.
7. **Gate A's mechanical diff-integrity scan is blocking** — LLM review and mechanical review are not interchangeable (per Expresso/Pod/Smartware history).
...
1. **`.factory/` config in the seed repo** — `PROJECT.md` (what/why, ≤100 lines), `AGENTS.md` (station role files: planner, builder, verifier + approval matrix + permission rules), `DESIGN.md` only if the repo has brand (else skip). Runtime adapters (Claude Code/Codex/Hermes) generated thin. No other state files.
...
| Learning gates (SOPs, skill auto-propagation, autoresearch-style loops) | ≥50 RunRecords AND rescue rate <30% |
